...
for vote,liste in groupe_dico_vote_nom.items():
    ...
    for elu in liste:
        ...
        print(acteurRef)
        mandatRef = elu.getchildren()[1].text
        if elu not in dico_elu_groupe:
            dico_elu_groupe[elu] = groupeRef
        elif not groupeRef in dico_elu_groupe[elu]:
            dico_elu_groupe[elu] += "/" + groupeRef
        if elu not in dico_elu_mandat:
            dico_elu_mandat[elu] = groupeRef
        elif not groupeRef in dico_elu_mandat[elu]:
            dico_elu_mandat[elu] += "/" + mandatRef
        dico_vote_liste[vote] += acteurRef + "/"
...
Pour la gestion des groupes politiques à l'AN : on va faire une liste dans groupe Ref pour les elu.

On fait un Dico avec tous les élus qui ont voté quelque chose dans lequel on ajoute "/" au début. Puis avant la MAJ dans la bd on enlève le premier "/".


Ordre des entrées dans la BD :
1) Organes
2) Srutins (pour que le dico Elu-> Groupe soit prêt)
3) Elu
